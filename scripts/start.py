...
# Lo script di training è train_core.py, che è anche lo script che stiamo modificando sotto
SCRIPT_PATH = CURRENT_SCRIPT_DIR / "train_core.py" 
...
def get_available_targets(required_subdir='8_dataset_split'):
    ...
    # Un target è valido se la sottocartella richiesta e il file train.json esistono
    valid_subdirs = [
...
def main():
    ...
    if not targets:
        print(f"❌ Nessun target con split JSON pronti trovato in {DATA_DIR}!")
        sys.exit(1)
    
    # 1. SELEZIONE TARGET(S)
    selected_target_names = select_targets_interactive(targets)
    # Convertiamo la lista in una stringa separata da virgole
    ...
    # Costruzione comando torchrun (utilizzato da torch.distributed.run)
    env = os.environ.copy()
    env["CUDA_VISIBLE_DEVICES"] = gpu_env_string
    # Parametri NCCL per stabilità su cluster/cloud
    ...
    cmd = [
        ...
        "--master_port=29500", # Porta fissa per DDP
        str(SCRIPT_PATH),
        "--target", target_env_string # Passa la stringa di target multipli
    ]
