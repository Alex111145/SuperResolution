...
import numpy as np
import matplotlib
# Backend grafico non interattivo (fondamentale per Linux/Server)
matplotlib.use('Agg') 
...
DEBUG_SAMPLES = 50 # Rimasto per config, ma non limita più i PNG
...
# --- UTILITY FITS LINUX (Case Insensitive) ---
def get_fits_files(folder):
    """Trova file FITS  gestendo maiuscole/minuscole."""
    files = list(folder.glob('*.fits')) + list(folder.glob('*.FITS')) + \
            list(folder.glob('*.fit')) + list(folder.glob('*.FIT'))
...
def create_aligned_lr_wcs(hr_patch_wcs, hr_size, lr_size):
    """Crea un WCS per la patch LR scalando esattamente da HR."""
    factor = hr_size / lr_size
    w_lr = hr_patch_wcs.deepcopy()
    
    if w_lr.wcs.has_cd(): w_lr.wcs.cd *= factor
    else: w_lr.wcs.cdelt *= factor
...
def process_single_patch_multi(args):
    ...
    for o_path in shared_data['o_files']:
        try:
            ...
            # Salvataggio FITS (Filename lowercase per Linux)
...
def main():
    print(f"ESTRAZIONE PATCH WCS-AWARE (LINUX VERSION)")
    ...
    h_files = get_fits_files(input_h)
    o_files_all = get_fits_files(input_o)
    ...
    # ProcessPoolExecutor su Linux usa fork() ed è molto efficiente
