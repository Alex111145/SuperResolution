    ...
    def __getitem__(self, idx):
        ...
        try:
            with fits.open(path) as hdul:
                # Assumiamo che i dati astronomici siano nel primo HDU
                data = hdul[0].data
                data = np.nan_to_num(data, nan=0.0, posinf=0.0, neginf=0.0)
                tensor = torch.from_numpy(data.astype(np.float32))
                return tensor
        except Exception:
            # Ritorna un tensore vuoto/di fallback in caso di errore di lettura
...
def calculate_robust_stats(file_list):
    ...
    for batch in tqdm(loader, desc=" 	 Sampling", ncols=100):
        # Applica Log Stretch (utile per comprimere il range dinamico dei dati astronomici)
        if USE_LOG_STRETCH:
...
def save_debug_png(hr_raw, lr_raw, hr_norm, lr_norm, clip_percentile, save_path):
    ...
    # RAW (Vista Log - per mostrare la distribuzione originale prima del clipping)
    ...
    # L'imshow viene forzata a visualizzare il range 0-65535 per rappresentare la 16-bit
...
def process_single_normalization(target_dir):
    ...
    for i, pair_path in enumerate(tqdm(all_pairs, ncols=100)):
        try:
            ...
            # Copie per il debug (dati RAW)
            raw_h_copy = d_h.copy()
            raw_o_copy = d_o.copy()

            # 1. Log Stretch (se attivo)
            if USE_LOG_STRETCH:
                d_h = np.log1p(np.maximum(d_h, 0))
                d_o = np.log1p(np.maximum(d_o, 0))

            # 2. Normalizzazione Globale (Min-Max con clipping)
...
def select_target_directories(required_subdir='6_patches_final'):
    ...
    # Controlla solo le directory che contengono i file FITS già processati
