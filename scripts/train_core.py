...
def train_worker():
    ...
    # Caricamento JSON e creazione temp file unici per processo per evitare conflitti
    ...
    # Importante: num_workers alto per nutrire le A40
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=False, 
                              num_workers=8, pin_memory=True, sampler=train_sampler, drop_last=True)
    # Val loader su distributed sampler per velocizzare (divide il lavoro)
    ...
    model = nn.SyncBatchNorm.convert_sync_batchnorm(model) # Sincronizza BN tra GPU
    ...
    for epoch in range(1, TOTAL_EPOCHS + 1):
        train_sampler.set_epoch(epoch) # Fondamentale per shuffle diverso ogni epoca
        model.train()
        acc_loss = 0.0
        optimizer.zero_grad()
        ...
        if epoch % LOG_INTERVAL == 0:
            # Calcola loss media su tutti i processi
            dist.barrier()
            loss_tensor = torch.tensor(acc_loss, device=device)
            dist.all_reduce(loss_tensor, op=dist.ReduceOp.SUM)
            avg_loss = loss_tensor.item() / (len(train_loader) * dist.get_world_size())
            ...
            if is_master:
                ...
                # Raccogli metriche dal master (stima) o implementa all_reduce per precisione assoluta
                # Qui usiamo solo master/local per velocità
                res = metrics.compute()
