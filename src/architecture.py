...
# Recuperiamo SwinIR (terzo valore restituito da env_setup per Swin)
...
class TrainHybridModel(nn.Module):
    def __init__(self, output_size=512, smoothing=None, device='cpu'):
        ...
        print("🏗️  Model Arch: SwinIR-Light (Vision Transformer) x4 [TRAIN MODE]")
        
        # CONFIGURAZIONE SWINIR
        self.model = SwinIR_Arch(
            ...
            # Parametri "Light" (adattati dal tuo config originale)
            depths=[6, 6, 8, 6],   
            embed_dim=90,          
            num_heads=[6, 6, 6, 6],
