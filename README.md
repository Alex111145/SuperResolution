...
Python 3.10+
ASTAP (Astrometric Solver): Download
...
python  embed_dim=120        # Divisibile per num_heads=6
...
pythonTotal Loss = λ₁·Charbonnier + λ₂·Perceptual + λ₃·Astro
...
bashpython Dataset_step1_datasetwcs.py
Cosa fa:
...
pythonFORCE_FOV = 0.46  # Gradi (adatta al tuo telescopio)
USE_MANUAL_FOV = True
Step 2: Quality Check (Opzionale)
bashpython Dataset_step2_mosaicHSTObs.py
...
bashpython Dataset_step3_extractpatches_Gaia.py
Cosa fa:
...
Parametri:
pythonHR_SIZE = 512      # Dimensione patch Hubble
...
bashpython Dataset_step4_normalization.py
Trasformazioni:
...
pythonTARGET_NAME = "M42"  # Nome del target
NUM_GPUS = 1         # Numero GPU
Step 3: Avvio Training
bashpython Modello_3.py
Hyperparameters (in Modello_supporto.py):
pythonBATCH_SIZE = 3          # Per H200 (141GB VRAM)
ACCUM_STEPS = 20        # Gradient Accumulation
LR = 4e-4               # Learning Rate
TOTAL_EPOCHS = 150
Monitoraggio con TensorBoard:
bashtensorboard --logdir=outputs/M42_GPU_0/tensorboard
Metriche tracciate:
...
bashpython Modello_4.py --target M42
...
Verifica FOV manuale:

python   FORCE_FOV = 0.46  # Calcola: (altezza_sensore_mm / focale_mm) * (180/π)
   USE_MANUAL_FOV = True

Controlla header FITS:
...
bash   python debugmodello.py

Verifica percentili:

python   LOWER_PERCENTILE = 1.0   # Aumenta se troppo scuro
...
python# In Modello_supporto.py (già implementato)
...
Verifica cartella models/:

bash   ls -la models/BasicSR/basicsr/archs/rrdbnet_arch.py
   ls -la models/HAT/hat/archs/hat_arch.py

Reinstalla:

bash   cd models/BasicSR && pip install -e . && cd ../..
...
Usa solo Stage 1 (disabilita HAT):

python   # architecture.py
   self.has_stage2 = False

Riduci dimensione patch:

python   HR_SIZE = 256    # Invece di 512
   AI_LR_SIZE = 64  # Invece di 128

📊 Risultati Attesi
Metriche Target
MetricaBaseline (Bicubic)Target ModelloPSNR~28 dB32-35 dBSSIM~0.850.92-0.95
Esempio Output
...
Repository: HAT Official



Dataset

Hubble Legacy Archive: HST Data
