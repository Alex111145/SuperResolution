...
class AntiCheckerboardLayer(nn.Module):
    def __init__(self, mode='balanced'):
        super().__init__()
        # Kernel ottimizzati per rimuovere la griglia tipica delle GAN/SuperRes
        if mode == 'strong':
            k, p, s = 7, 3, 1600.0
...
class TrainHybridModel(nn.Module):
    def __init__(self, output_size=512, smoothing='balanced', device='cpu'):
        ...
        # STAGE 2: HAT (Raffinamento Texture)
        if HAT_Arch:
            try:
                ...
                print("   🧠 [TRAIN ARCH] HAT Inizializzato (Config: Full Precision).")
            except Exception as e:
                print(f"   ⚠️ HAT Error: {e}")

        # Smoothing attivo per evitare artefatti durante il training prolungato
        if smoothing != 'none':
